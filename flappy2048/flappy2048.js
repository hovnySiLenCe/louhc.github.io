 ...
 */
 ...
 for (var i = 0; i<scpts.length; i++)="" {="" var="" scpt="scpts[i];" xx="scpt.x;" yy="scpt.y;" div="score.divs[i];" div.style.left="Math.round(xx" +="" (ww="" -="" 300)="" 2)+'px';="" div.style.top="(yy" 10)+'px';="" }="" want_image_count="0;" loadgameimage="function" (n)="" want_image_count++;="" o="document.createElement('img');" o.onload="function" ()="" want_image_count--;="" if="" ((want_image_count="=" 0)="" &&="" (want_sound_count="=" 0))="" gameloaded();="" o.src="n;" return="" o;="" sounds_playing="[];" want_sound_count="0;" loadgamesound="function" (n,="" chc)="" file="" name,="" channel="" count="" o.channels="[];" o.channel_pos="0;" o.chc="chc;" o.play="function" (!sndo.has_sound)="" return;="" tm="new" date().gettime();="" sc="this.channels.length;" got_good_sound="false;" ch;="" for="" (var="" i="sc;">=0; i--) {
   this.channel_pos++;
 ...
 for (var i = 0; i<chc; i++)="" {="" var="" ao="document.createElement("audio");" sch="{};" sch.sound="ao;" sch.can_play="true;" o.channels.push(sch);="" ao.src="n;" ao.type="audio/wav" ;="" document.body.appendchild(ao);="" }="" return="" o;="" game_loaded="false;" gameloaded="function" ()="" game.div.appendchild(game.bg);="" game.div.appendchild(ground.canvas);="" game.div.appendchild(logo.img);="" game.div.appendchild(gameover.img);="" document.body.appendchild(fbshare.img);="" document.body.appendchild(playagain.img);="" game.div.appendchild(c2s.img);="" game.div.appendchild(sndo.div);="" game.div.appendchild(parody.div);="" for="" (var="" i="score.divs.length-1;">=0; i--) {
...
var scpts = [];
var rad = 4;
for (var xx = -rad; xx<=rad; xx++)="" {="" for="" (var="" yy="-rad;" yy<="rad;" yy++)="" var="" d="Math.sqrt(xx*xx" +="" yy*yy);="" if="" (d="" <="rad)" o="{};" o.x="xx;" o.y="yy;" scpts.push(o);="" }="" score="{};" score.divs="[];" i="0;" i<scpts.length;="" i++)="" scpt="scpts[i];" xx="scpt.x;" div="document.createElement('div');" score.divs.push(div);="" div.style.width="300px" ;="" div.style.height="60px" div.style.position="absolute" div.style.textalign="center" div.style.fontfamily=""Clear Sans", "Helvetica Neue", Arial, sans-serif" div.style.fontweight="bold" div.style.fontsize="40px" div.style.color="#faf8ef" div.style.zindex="88887;" div.style['user-select']="none" div.style['-webkit-user-select']="none" div.style['-moz-user-select']="none" div.style['-ms-user-select']="none" div.style['-o-user-select']="none" *="" -webkit-user-select:="" none;="" chrome="" safari="" -moz-user-select:="" firefox="" -ms-user-select:="" ie10+="" rules="" below="" not="" implemented="" in="" browsers="" yet="" -o-user-select:="" user-select:="" ((xx="=" 0)="" &&="" (yy="=" 0))="" div.innerhtml="" score.update="function" ()="">=0; i--) {
...
var getValueStr = function (num) {
 while (vstrs.length <= num)="" {="" var="" s="vstrs[vstrs.length" -="" 1];="" n="0;" r="0;" fs="" ;="" for="" (var="" i="s.length-1;">=0; i--) {
   n = Number(""+s.charAt(i));
   n *= 2;
   n += r;
   r = Math.floor(n/10);
...
var newWall = function (wall_val) {
 ...
 var maxlen = 0;
 var cells = wall.cells;
 for (var i = 0; i<wall_cell_count; i++)="" {="" var="" cell="newCell();" cell.x="wall.x;" cell.sty="(i+1)*(cell_size" +="" 14);="" cell.y="(hh-88)" -="" cell.sty;="" cell.sy="cell.y;" cell.vx="wall.vx;" game.div.appendchild(cell.div);="" cells.push(cell);="" cell.setvalue(wall_val);="" math.max(1,="" game.cur_wall_val="" math.floor(math.random()*4)="" 2));="" maxlen="Math.max(maxlen," cell.vs.length);="" if="" ((cell.y="" <="0)" &&="" (cells.length="">= 4)) {
   break;
  }
 }
 ...
 if (v <= 1)="" v="1;" for="" (var="" i="cells.length-1;">=1; i--) {
  v++;
  if (v == wall_val) v += 2;
  //if (v == right_value) v++;
  vals.push(v);
...
var oef = function () {
 if (game_loaded) {
  ...
  while (ltm < ftm) {
   ...
   if (logo.hiding) {
    logo.a -= .05;
    if (logo.a <= 0)="" {="" logo.a="0;" logo.hiding="false;" }="" logo.img.style.opacity="logo.a;" if="" (logo.a=""> 0) {
    logo.fr++;
    logo.reposition();
   }
   if (logo.a > .01) {
   ...
   if (gameover.hiding) {
    gameover.a -= .1;
    if (gameover.a <= 0)="" {="" gameover.a="0;" gameover.hiding="false;" }="" gameover.img.style.opacity="gameover.a;" if="" (gameover.a=""> 0) {
    gameover.fr++;
    gameover.reposition();
   }
   if (gameover.a > .01) {
   ...
   if (fbshare.hiding) {
    fbshare.a -= .1;
    if (fbshare.a <= 0)="" {="" fbshare.a="0;" fbshare.hiding="false;" }="" fbshare.img.style.opacity="fbshare.a;" if="" (fbshare.a=""> 0) {
    fbshare.fr++;
    fbshare.reposition();
   }
   if (fbshare.a > .01) {
   ...
   if (playagain.hiding) {
    playagain.a -= .1;
    if (playagain.a <= 0)="" {="" playagain.a="0;" playagain.hiding="false;" }="" playagain.img.style.opacity="playagain.a;" if="" (playagain.a=""> 0) {
    playagain.fr++;
    playagain.reposition();
   }
   if (playagain.a > .01) {
   ...
   if (c2s.hiding) {
    c2s.a -= .1;
    if (c2s.a <= 0)="" {="" c2s.a="0;" c2s.hiding="false;" }="" c2s.img.style.opacity="c2s.a;" if="" (c2s.a=""> .01) {
    if (!c2s.visible) {
     c2s.visible = true;
   ...
   if (game.started) {
    ...
    for (var i = walls.length-1; i>=0; i--) {
     ...
     if (!wall.passed) {
      ...
      if (clc == null) {
       if (wall.value <= 3)="" {="" for="" (var="" j="0;" j<cells.length;="" j++)="" cells.length-1;="">=0; j--) {
         var cell = cells[j];
    ...
    if (doubling) {
     ...
     doubling_cell.a -= .15;
     if (doubling_cell.a <= 0)="" {="" doubling_cell.a="0;" wedc++;="" }="" doubling_cell.dsa="" -=".15;" if="" (doubling_cell.dsa="" <="0)" var="" dsz=".5*Math.sin(Math.PI*doubling_cell.dsa);" bird.dsz="dsz;" bird.div.style.width="(doubling_cell.ww" +="" dsz)+'px';="" bird.div.style.height="(cell_size" bird.div.style.lineheight="(cell_size" for="" vertical="" align="" of="" text="" doubling_cell.div.style.opacity="Math.floor(doubling_cell.a*100)/100;" (wedc="=" 2)="" all="" conditions="" requiring="" doubling="" to="" end="" are="" true="" wedc="0;" ((!touching_wall)="" &&="" (!doubling))="" bird.vvy="" (bird.vvy=""> 1) bird.vvy = 1;
     bird.vy += .65 * bird.vvy;
...
window.onresize = function () {
 resize();
}</=></=></=></=></=></=></=></=></wall_cell_count;></=></=rad;></chc;></scpts.length;>